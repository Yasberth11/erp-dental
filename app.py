...
def vista_consultorio():
    ...
    with st.sidebar.expander("🛠️ Mantenimiento"):
        if st.button("🗑️ RESETEAR BASE DE DATOS", type="primary"):
            ...
            st.cache_data.clear() # ¡AQUÍ ESTÁ LA SOLUCIÓN AL "ZOMBIE"!
            st.success("Sistema limpio."); time.sleep(1); st.rerun()
    ...
    if menu == "Agenda":
        ...
        with tab1:
            with st.form("alta", clear_on_submit=False):
                ...
                rfc = st.text_input("RFC (Automático)", disabled=True) # SE QUEDA VACÍO Y SE CALCULA AL GUARDAR
                st.markdown("---")
                ...
                if st.form_submit_button("Guardar"):
                    if n and p and len(tel)==10:
                        ...
                        rfc_calc = calcular_rfc_10(n, p, m, fn) # CÁLCULO AQUÍ
                        c = conn.cursor()
        ...
        if not p_raw.empty:
            ...
            # --- SOLUCIÓN FIRMAS GRISES (KEYS ÚNICOS) ---
            with st.expander("Firma Paciente", expanded=True): canvas_pac = st_canvas(stroke_width=2, height=100, key="c_pac")
            with st.expander("Firma Doctor"): canvas_doc = st_canvas(stroke_width=2, height=100, key="c_doc")
            with st.expander("Testigo 1"): canvas_t1 = st_canvas(stroke_width=2, height=100, key="c_t1")
            with st.expander("Testigo 2"): canvas_t2 = st_canvas(stroke_width=2, height=100, key="c_t2")
            
            if st.button("Generar PDF Legal"):
                # CAPTURA DE LAS 4 IMÁGENES (O None si están vacías)
                def cap(cv): return base64.b64encode(io.BytesIO(Image.fromarray(cv.image_data.astype('uint8'), 'RGBA').save(io.BytesIO(), format='PNG') or b'').getvalue()).decode() if cv.image_data is not None else None
