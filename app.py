...
# --- FUNCIÓN ASISTENCIA (MOVIDA AQUÍ PARA EVITAR ERROR) ---
...
class PDFGenerator(FPDF):
    ...
    def header(self):
        ...
        self.set_font('Arial', 'B', 15); self.set_text_color(0, 43, 91)
...
def crear_pdf_consentimiento(paciente, doctor, tipo_doc, tratamiento, firma_img_data):
    ...
    if "Aviso" in tipo_doc:
        ...
        cuerpo = f"""FECHA: {datetime.now().strftime("%d/%m/%Y")}\n\nPACIENTE TITULAR: {paciente}\n\nDe conformidad con la Ley Federal de Protección de Datos Personales en Posesión de los Particulares, Royal Dental hace de su conocimiento que sus datos personales y sensibles serán tratados con estricta confidencialidad para fines clínicos, administrativos y estadísticos.\n\nUsted tiene derecho a Acceder, Rectificar, Cancelar u Oponerse (ARCO) al tratamiento de sus datos."""
    else:
...
def pantalla_login():
    ...
    with col_centro:
        # MEJORA ESTÉTICA: LOGO CENTRADO Y SIN FONDO AZUL GIGANTE
...
def vista_consultorio():
    ...
    if menu == "1. Agenda & Citas":
        ...
        if not df_p.empty:
            ...
            tipo_doc = st.selectbox("Documento", ["Consentimiento General", "Aviso Privacidad", "Consentimiento Cirugía"])
            doc_sel = st.selectbox("Doctor", ["Dr. Emmanuel", "Dra. Mónica"])
            
            # CORRECCIÓN: DEFINIR CANVAS PRIMERO
            canvas_result = st_canvas(stroke_width=2, height=150, key="canvas")
            
            if st.button("Generar PDF"):
                # AHORA SÍ LEEMOS LA VARIABLE EXISTENTE
